...
4. Confirm `Save & Next` returns to the live camera with the next item number and a cleared draft.
5. Confirm `Submit` reaches the existing queue review path without touching DB submit.
6. Confirm Capture Home `Upload Batch` refuses to submit when a current draft still has photos and shows a finish-current-item message.
7. Relaunch without the launch argument and confirm normal `Open Camera` and `Preview Intake Flow` behavior still work.
