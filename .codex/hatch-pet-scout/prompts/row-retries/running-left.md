Create Codex pet row `running-left` for `scout`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact fox mascot with camera-lens eyes, a small crossbody satchel, and a tidy courier-like vibe inspired by item photography and review work; plush or sticker-friendly, readable at sprite size.. Preserve silhouette, face, palette, material, proportions, markings, and props.
