Create Codex pet row `running-right` for `pavo`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact peacock mascot with a rounded fan-tail, jewel-toned plumage, and a poised helper vibe inspired by item photography and review work; plush or sticker-friendly, readable at sprite size.. Preserve silhouette, face, palette, material, proportions, markings, and props.
